...
public class GlobalDataService {
    ...
    @Autowired
    public GlobalDataService(DownloadService downloadService, Nc4Manager nc4Manager, NetCdfProcessor netCdfProcessor, LocalDataDao localDataDao, PositionService positionService, UserService userService, UpdateProcessRepository processRepository) {
    ...
    public void updateData(NasaServer nasaServer, String startDate, String endDate) {
        LocalDate startDateLD = LocalDate.parse(startDate);
        LocalDate endDateDT = LocalDate.parse(endDate);
        if (startDateLD.isAfter(endDateDT) || startDateLD.equals(endDateDT)) {
            throw new IllegalArgumentException("Start date can't be after or equals end date");
        }
        int days = Period.between(startDateLD, endDateDT).getDays();

        Stream.iterate(startDateLD, date -> date.plusDays(1))
                .limit(days)
                .forEach((date) -> {
